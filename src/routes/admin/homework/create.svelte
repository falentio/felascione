...
<section class="p-4 flex flex-col gap-4">
	<form class="flex flex-col p-4 gap-2 border border-black bg-sky-200">
		<div class="flex flex-col">
			...
			<input id="subject-field" type="text" name="subject" bind:value={subject} class="bg-gray-100 ring-blue-500 focus:ring-2 outline-none rounded-sm px-1" />
		</div>
		<div class="flex flex-row flex-wrap gap-2">
			<label for="dueDate-field" class="w-full"> Due Date </label>
			<input id="dueDate-field" type="date" name="dueDate" bind:value={dueDate} class="appearance-none w-max" />
			<button
				type="button"
		...
		<ul class="flex flex-col gap-1">
			...
			{#each attatchments as a, i}
				<li class="flex gap-2">
					<a href={a.url} class="flex-auto overflow-hidden truncate bg-pastel-yellow rounded-sm shadow-md"> {a.url} </a>
					<button
						type="button"
	...
	{#if homework}
		...
		{:catch e}
			<div class="block border border-black p-2 bg-red-200"> Failed to create homework. {e.message || ":("} </div>
		{/await}
	{/if}
</section>